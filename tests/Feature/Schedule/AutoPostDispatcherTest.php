...
it('claims a due slot atomically and queues one job per enabled platform', function (): void {
    ...
    Queue::assertPushed(fn (PostSlotToPlatform $job): bool => $job->slotId === $slot->id && $job->platform === 'youtube');
    Queue::assertPushed(fn (PostSlotToPlatform $job): bool => $job->slotId === $slot->id && $job->platform === 'tiktok');

    // Segundo disparo (tick duplicado / clique) perde o claim.
    expect(resolve(AutoPostDispatcher::class)->dispatchSlot($slot->refresh()))->toBeFalse();
    Queue::assertPushed(PostSlotToPlatform::class, 2);
});
