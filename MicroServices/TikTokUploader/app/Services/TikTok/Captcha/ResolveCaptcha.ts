...
import type { ImagePlacement, InferenceResult } from '@/Types/CaptchaType';
...
function objectForQuestion(question: string): string | null {
...
function toWebpageCoordinates(boxes: BoundingBox[], placement: ImagePlacement): Point[] {
...
async function estimateRotationAngle(page: Page): Promise<RotationEstimate> {
    return page.evaluate((selector: string): RotationEstimate => {
        const images = [...document.querySelectorAll<HTMLImageElement>(selector)].filter(
        ...
        const pixelsFor = (image: HTMLImageElement, angle: number): Uint8ClampedArray => {
            context.clearRect(0, 0, sampleSize, sampleSize);
            context.save();
            context.translate(sampleSize / 2, sampleSize / 2);
            context.rotate((angle * Math.PI) / 180);
            context.drawImage(image, -sampleSize / 2, -sampleSize / 2, sampleSize, sampleSize);
            context.restore();
        ...
        const score = (a: Uint8ClampedArray, b: Uint8ClampedArray): number => {
            let total = 0;
            let count = 0;
