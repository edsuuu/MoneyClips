...
.PHONY: up setup check setup-laravel setup-download setup-tiktok setup-video setup-autocaption
...
up:  ## Sobe Laravel + microserviços nativos, todos juntos
	npx concurrently -k \
		...
		-n serve,queue,pail,vite,download,tiktok,video,autocaption \
		"php artisan serve" \
...
setup: setup-laravel setup-download setup-tiktok setup-video setup-autocaption  ## Instala deps + copia .env de tudo
